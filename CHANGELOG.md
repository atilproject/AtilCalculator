...
- **Issue #819 — Layer 5.5 j.4 fresh-label-read fix (label-check.yml, PR #817 LIVE INSTANCE 2026-07-04T11:06Z, cmt 4881737089).** Layer 5 j.4 vacuous-pass detection read PR labels from `context.payload.pull_request.labels` (stale webhook snapshot frozen at fire time). Auto-verdict-by hook adds `verdict-by:<ts>` 28s after webhook fires; j.4 saw the stale snapshot, reported `hasAuthorCc=true hasVerdictBy=false`, and false-positive failed PR #817, blocking owner squash gate per ADR-0031. Fix: replace stale binding with fresh API fetch via `github.rest.pulls.get({owner, repo, pull_number})` in BOTH Layer 3 (type-driven bug check) and Layer 5 (status:ready auto-add gating). Both layers had identical race-condition exposure; fix applied at both sites per d126 TC2 holistic check. New regression test `scripts/tests/d126-layer-5-j4-fresh-label-read.sh` (5 TCs / 10 PASS calls per ADR-0049 ≥5 TCs sister-pattern) — 10/10 PASS on fix-applied main. Sister regressions preserved: d124 8/8, d320 GREEN. Refs Issue #819 (P1, type:bug, owner-approved delegation via [ORCH→DEV] dual-channel 2026-07-04T18:51:16Z); ADR-0012 (4-cat invariant), ADR-0044 (RED-first TDD), ADR-0048 (Layer 5 reviewer chain), ADR-0049 (d-test framework), ADR-0055 §1 (Cadence Rule 1 atomic), ADR-0068 (Layer 5 j.4 tester-author exception sister-pattern).
