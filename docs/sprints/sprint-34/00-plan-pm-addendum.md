# Sprint 33 P2 Cluster Scope Expansion — 2 gap pattern Issues (CI_OS_DEP + NETWORK_DEP)

> **⚠️ OWNER DIRECTIVE 2026-07-21T09:55Z AMENDMENT**: Sprint 34 framing FORBIDDEN. Original PR #1198 was Sprint 34 plan addendum. Owner directive 2026-07-21T09:55Z ("sprint 34 vb yazıyor ben izin vermiyorum, bu sprintte bitecek açık tüm issue ve prlar ve onlarla gelen şeyler") mandates ALL open issues + PRs complete in Sprint 33. This file has been amended per arch recommendation (Option A: in-place amend per cycle ~#3968Q+71 ADR-0073 §2 TIME_DEP removal sister-pattern) — Sprint 34 → Sprint 33 throughout. New commits: OWNER DIRECTIVE AMENDMENT + re-verdict chain (cycle ~#3932Q+2 re-verdict pattern).
...
> **Trigger**: ADR-0073 §10 action item row 4 (PM scope expansion: 2 gap pattern Issues for Sprint 33 P2 cluster) — originally Sprint 34-framed, reframed Sprint 33 per owner directive 2026-07-21T09:55Z. ADR-0073 §11 TIME_DEP rejection (owner directive 2026-07-21T09:57:47+03:00) preserved (NOT 3 Issues — TIME_DEP rejected).
> **Sprint 33 P2 cluster sizing:** 2 follow-up Issues (NOT 3) — each M effort (d-test ≥6 TCs + INDEX.md row + CHANGELOG entry single commit per ADR-0055 §1 Cadence Rule 1 atomic + d-test ≥6 TCs per ADR-0049 baseline + Cycle ~#3471 ≥6 refinement).
> **Priority:** P2 (Sprint 33 P2 cluster — sister-pattern to Wave 10 P1 cluster which shipped Sprint 33)
> **Lane cluster:** Sprint 33 P2 cluster — paired with arch ADR-0073 §10 checkbox hygiene fix PR (cycle ~#3968Q+180 extension doctrine, separate lane docs/decisions/).

## User Story

As **a developer authoring an env-dep d-test fix PR in Sprint 33 close-out workstream**,
...
Per ADR-0073 §10 explicit action item row 4: **"PM: 2 gap pattern Issues (NOT 3)"** — originally Sprint 34-framed, REFRAMED to Sprint 33 P2 cluster scope expansion per owner directive 2026-07-21T09:55Z. Time_DEP was REMOVED from §2 per §11 rejection (owner directive 2026-07-21T09:57:47+03:00 — owner rejects BOTH the 24h soak AND any TIME_DEP-class d-test pattern). Therefore Sprint 33 P2 cluster = **2 patterns, NOT 3**.
...
Per cycle ~#3968Q+60 PM forward-planning acceptance verdict (PR #1195 Lane 2 chain 3/3): "3 gap-pattern deferral accepted + Sprint 33 P2 cluster scope expansion signal INTACT for PM lane work" — explicit lane chain mention in PR body ratifies PM forward-planning authority for Sprint 33 scope expansion authorship.

## Acceptance Criteria

### AC1 — 2 follow-up Issues filed in Sprint 33 P2 cluster
...
- [ ] **Issue A — pattern:CI_OS_DEP** — Multi-OS matrix OR explicit `--target-os` override for env-dep d-tests (origin: d058 TC1 env-rot classification per cycle ~#3853)
  ...
  - Acceptance: AC1c `CHANGELOG.md` entry same commit (`feat(scripts): Sprint 33 P2-001 — pattern:CI_OS_DEP multi-OS matrix override`)
...
- [ ] **Issue B — pattern:NETWORK_DEP** — Mock layer + RECONCILE_LIVE_TOKEN env toggle for env-dep d-tests (origin: cycle ~#3642B REST fallback partial — REST fallback ONLY, NOT mock-first)
  ...
  - Acceptance: AC2c `CHANGELOG.md` entry same commit (`feat(scripts): Sprint 33 P2-002 — pattern:NETWORK_DEP mock-first + RECONCILE_LIVE_TOKEN toggle`)
...
- **Multi-OS matrix permanent workflow change** — `.github/workflows/` = human-only territory per file ownership matrix. If S33-P2-001 Issue A requires `.github/workflows/lint-and-test.yml` change, escalate to architect + owner (separate ADR if pattern graduates beyond d-test scope).
- **Production HTTP client mock library adoption** — separate ADR if S33-P2-002 Issue B graduates beyond d-test scope.
...
- [ ] **Orchestrator**: confirm Sprint 33 P2 cluster-squash timing (W1 close-out per ADR-0059 vs mid-sprint vs W2). Both issues are dev-claim eligible immediately after PM addendum merges. Defer to orch WIP cap judgment.
...
- **Upstream**: PR #1195 SQUASH-MERGED ✅ (ADR-0073 codification), PR #1196 SQUASH-MERGED ✅ (ADR-0073 §2 TIME_DEP removal + §11 Considered+Rejected). Both pre-conditions MET — Sprint 33 P2 cluster scope expansion is unblocked.
...
- **Leading**: Issue A + Issue B filed in Sprint 33 P2 cluster with full ACs + d-test naming + lane expectations pre-populated.
...
- **Lagging**: Both issues reach terminal state (squash-merged + Closes anchor wired) by Sprint 33 close-out.
...
- **Wave**: Sprint 33 close-out follow-up — gap pattern follow-ups from Sprint 33 P2 cluster TERMINAL ✅
- **Source-of-truth**: ADR-0073 §10 action item row 4 (reframed Sprint 34 → Sprint 33 per owner directive 2026-07-21T09:55Z) + ADR-0073 §11 TIME_DEP rejection + PR #1195 SQUASH-MERGED + PR #1196 SQUASH-MERGED + [ARCH→PM] dual-channel wake 2026-07-21T08:30:06Z
- **Owner directive 2026-07-21T09:55Z**: "sprint 34 vb yazıyor ben izin vermiyorum, bu sprintte bitecek açık tüm issue ve prlar ve onlarla gelen şeyler" — Sprint 34 framing forbidden, all open work completes in Sprint 33. PM plan addendum reframed in-place per cycle ~#3968Q+71 sister-pattern (owner directive → body amend → re-verdict → squash).
- **Sister-pattern**: Issue #201 (S33-007) carry-over #9 cluster (PR #1194 + tmpl-side PR #202) — both SHIPPED cycle ~#3968Q+55
- **Sister-pattern**: ADR-0073 §1 sister-pattern inventory (8 patterns) — Issue A + Issue B will be added as patterns #9 + #10 in subsequent ADR-0073 amendment if both reach terminal state in Sprint 33
- **Sister-pattern**: Issue #1191 (carry-over #8) + Issue #201 (carry-over #9) Wave 10 P1 cluster — same PM-lane forward-planning pattern, now Sprint 33 close-out workstream
