...
| Wave 1 step 2: S25-002 (carryover) d-test RED-first | 🟡 **IN REVIEW** | tester | PR #948 (draft, +541/-0, 3 files) | Test plan + d-test (9 TCs, TC1-TC9) + scripts/tests/INDEX.md (Cadence Rule 1 atomic per ADR-0055). Sister-pattern d068-td067-combined.sh (closed-axis sister, 7 TCs). Pre-impl expectation: TC1+TC3+TC4+TC5+TC6 FAIL (RED per ADR-0044), TC2+TC7+TC8+TC9 PASS (baseline-preserved). needs-architect-review on (D2.2 wake). Per ADR-0044 RED-first, blocks S25-001 impl PR (step 3). S25-002 = carryover ID per PM Option 1. |
