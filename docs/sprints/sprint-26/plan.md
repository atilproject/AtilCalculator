...
|---|---|---|---|---|
| Wave 1 step 1: TD-067c design phase | 🟡 **LINK FIX PUSHED, CI IN_PROGRESS** | architect | PR #946 (draft, +572/-2, 4 files, 4 commits) | 4 commits on `arch/td-067c-design-issue-931`: c8b29718c8 (original ADR-0071+design), 5322edc5 (S26-*→S25-* rebind), a56bb7e (6 broken links fixed), b7288f5 (ADR-0057 → amendment-closes-vs-refs-intent Amendment #1). All 7 slugs in ADR-0071 Related paragraph resolved correctly. 9-Lens pre-publish attestation per ADR-0045 ✅. 4 LIVE INSTANCES evidence stack. Sister-pattern to TD-067b Layer 6 (PR #938). **Lint & Test check IN_PROGRESS** job 86187855584 (dispatch discipline — NOT flipping status:ready until COMPLETED+SUCCESS, retrospect from PR #948 part 7 violation). d058 + d064 SUCCESS. Awaiting owner squash gate per ADR-0031. |
...
| Wave 1 step 2: S25-002 (carryover) d-test RED-first | 🟡 **CI FLAKE tracked in #949 (dev triage pending)** | tester | PR #948 (draft, +541/-0, 3 files) | Test plan + d-test (9 TCs, TC1-TC9) + scripts/tests/INDEX.md (Cadence Rule 1 atomic per ADR-0055). Sister-pattern d068-td067-combined.sh (closed-axis sister, 7 TCs). Pre-impl expectation: TC1+TC3+TC4+TC5+TC6 FAIL (RED per ADR-0044), TC2+TC7+TC8+TC9 PASS (baseline-preserved). Arch 🟢 (cmt 4927781958) + Tester 🟢 (cmt 4927820539) + Arch ACK (cmt 4927837459) = 🟢🟢 converged. **CI red on pre-existing perf flake** `test_arithmetic_p99_under_50ms_still_holds` (PR #948: 724.50ms / PR #947: 502.52ms vs 500ms budget). **Issue #949 filed by tester** (P3, transient, +0.5% over budget, model bug report with 3 root-cause hypotheses ranked, sister-pattern d112 + TD-049 deferred). Per ADR-0051 doctrine: flake must be tracked (✅ via #949) but doesn't block merge if waiver documented. **Dispatch Discipline step 3 violation caught** (cmt 4927888825 revert) — RETRO-018 candidate. Awaiting dev triage on #949 (a) BUDGET_MULTIPLIER bump 10.0→12.0 quick fix, (b) TD-049 Sprint 27 defer, (c) explicit CI waiver per ADR-0051. S25-001 impl PR unblock contingent on PR #948 CI clear OR #949 waiver. S25-002 = carryover ID per PM Option 1. |
...
| Owner squash gate (Sprint 26 wave 1) | ⏸️ **PENDING** | human | **Squash-sequence (4 PRs, cmt 4927849737):** PR #946 → main FIRST (after arch link fix cmt 4927769597), PR #948 → main SECOND, PR #947 → main THIRD (arch content no-op), PR #945 → main LAST (after PM S26-001 reconciliation) | Per ADR-0031. **Sequencing disclosure** (arch cmt 4927706895): PR #946 squash-first lands 4 arch files on main; PR #947 squash-second becomes no-op for arch content. PR #948 is independent d-test (no arch content), so it can squash in any order relative to #946/#947 but should land BEFORE S25-001 impl PR (RED-first per ADR-0044). Designer-only territory (workflow YAML touches `.github/workflows/`). |
...
- **Issue #949** — BUG filed by tester: test_arithmetic_p99_under_50ms_still_holds flake (P3, transient, +0.5% over budget, 3 root-cause hypotheses ranked, sister-pattern d112 + TD-049 deferred, awaiting dev triage)
