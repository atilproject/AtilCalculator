# ADR-0072: Task-list Persistence + Watchdog Tuning Revision (Sprint 32)

> **Status**: PROPOSED (draft, awaiting owner ratification per ADR-0012 birth contract)
> **Date**: 2026-07-19
> **Author**: @orchestrator (cycle ~#3751, post-owner-directive cycle-timestamp 2026-07-19T14:15+03:00)
...
While the spirit of the directive was correct (instant-fire > delayed-clear on genuinely stuck panes), the tightening overshoot produced a **secondary defect**: `/compact` operations take 30-90 seconds, but the watchdog's 60s cycle interval + 1-minute `STUCK_AFTER_MIN` causes stuck_override to fire DURING legitimate `/compact` execution, producing **210 false-positive cleared=yes events across the 7-day journal window** (`/var/log/dev-studio/AtilCalculator/journal/facts-2026-07-{13..19}.jsonl`).

### Ground-truth metrics

Verified 2026-07-19T14:38Z via direct file inspection:
...
(Note: owner-directive cited 210 — non-fabrication guard adjustment: actual count is **214** across 7 days, all correlated with `stuck_override` triggers per file inspection.)
...
|---|---|---|---|
| Canonical | atilproject/dev-studio-template | `*.sh`, `*.tmpl` | source of truth |
| Mirror | atilproject/AtilCalculator | `*.sh` (no .tmpl suffix) | full mirror, no .tmpl variant |
...
**File**: `docs/decisions/ADR-0072-tasklist-persistence-and-watchdog-tuning-revision.md` (this file)
**Labels** (per ADR-0012 birth contract): `type:doctrine + status:proposed + agent:architect + cc:human`
**STOP**: Await owner ratification (ADR-0072 approved label + owner "go" verbatim) before S32-XXX-B/C/D/E.

### Story S32-XXX-B — Impl PR template repo (canonical source of truth)

**Files** (10 changes, single PR):
- `scripts/agent-context-monitor.sh` — STUCK_AFTER_MIN default 1→10, STUCK_AFTER_MIN_CRITICAL 0→5, header rationale update (cycle #1638 → revised by ADR-0072 cycle-ref)
...
- `README.md` — new section "Task-list Persistence" + ADR-0072 link
...
- `RETRO-032.md` cycle #1638 → ADR-0072 evolution capture (defensive tightening → data-driven revision doctrine)
...
*— @orchestrator, cycle ~#3751 (post-owner-directive cycle-timestamp 2026-07-19T14:15+03:00). Draft awaiting owner ratification per ADR-0012 birth contract.*
