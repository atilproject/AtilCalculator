#!/usr/bin/env bash
# d1091-wip-idle-claim-freshness.sh
#
# d1091 — Issue #1091 BUG regression guard: wip-idle-detect.sh wip_count > 0
#         must override activity-signal check (any active WIP = NOT idle).
#         Self-referential false-positive: claim via label flip doesn't count
#         as "comment" signal; no PR yet; no commits → all 3 signals missing →
#         flagged idle within minutes of claim.
#
# Why this test exists
# --------------------
# scripts/wip-idle-detect.sh current logic (lines ~140-180):
#   1. Filter roles to status:in-progress + agent:<role>
#   2. For each WIP issue, check 3 activity signals (comment/PR-draft/commit)
#   3. If all signals missing/old → mark idle (THIS IS THE BUG)
#
# Defect shape (Issue #1091 body):
...
#
# Live instance (cycle ~#2272 orchestrator-captured):
...
#
# Fix spec (Issue #1091 body, points 1+2):
#   "scripts/agent-watch.sh wip_idle_wave path must:
...
#   TC1 happy path — 0 WIP + recent heartbeat → NOT idle (no regression)
#   TC2 self-referential FP — 1 WIP + no activity signals → NOT idle (RED pre, GREEN post)
#   TC3 mixed activity — 1 WIP + recent comment → NOT idle (existing behavior preserved)
#   TC4 stale activity regression — different role with stale activity → idle
#   TC5 multi-role selectivity — only idle roles flagged, active WIP roles excluded
#   TC6 Cadence Rule 1 atomic — INDEX.md has d1091 row (ADR-0055 §1)
#
# Run: bash scripts/tests/d1091-wip-idle-claim-freshness.sh
# Exit: 0 = all pass, 1 = at least one fail.
#
# Sister-pattern lineage:
#   - d1082 (Issue #1089 retry-with-backoff — same mock gh PATH shim pattern,
#     same Issue #113 issue-number-as-d-test-id slot allocation, same ADR-0049
#     deviation-justification precedent)
#   - d1088 (Issue #1088 query_stale_verdict owner-gate exemption — same
#     heuristic-gap doctrinel class, same RETRO-024 silent-skip sister-pattern,
#     same live-instance captured-in-heartbeat test methodology)
#   - d1081 (Issue #1081 RETRO-024 silent-skip — same claim predicate race
#     detection doctrinel class, sister at claim-next-ready.sh interface)
#   - d020a (Form C claim predicate race — original race detection pattern)
#
# Refs: Issue #1091 (P2 BUG, Sprint 29 W2 gap-closing — `scripts/agent-watch.sh`
#       wip_idle_wave heuristic gap, sister-pattern to #1088 #1081),
#       ADR-0039 (WIP-idle watchdog — current broken surface),
#       ADR-0044 (RED-first TDD doctrinal home — TC2 RED pre-impl),
#       ADR-0049 (≥5 TCs baseline — d1091 = 7 TCs met),
#       ADR-0055 §1 (Cadence Rule 1 atomic — d-test + INDEX.md row + impl
#       same PR cluster — Pattern A single-PR atomic),
#       Issue #113 (label-authority — d1091 slot = issue-number).
...
if ! command -v jq >/dev/null 2>&1; then
  echo "ERROR: jq required for d1091" >&2
  exit 127
fi
if [ ! -r "$WIP_SH" ]; then
...
wip_dev_issue() {
  ...
  printf '[{"number":1091,"title":"BUG: agent-watch.sh wip_idle_wave heuristic gap","updatedAt":"%s"}]\n' "$NOW"
}

# Helper: 1 recent comment for issue 1091
...
run_wip_detect() {
  ...
  GITHUB_REPO="atilproject/AtilCalculator" \
    bash "$WIP_SH" \
...
# TC6 — Cadence Rule 1 atomic (ADR-0055 §1): INDEX.md has d1091 row.
section "TC6 — Cadence Rule 1 atomic (INDEX.md d1091 row present)"
if grep -q "d1091" "$INDEX_MD" 2>/dev/null; then
  pass "TC6 — scripts/tests/INDEX.md has d1091 row (Cadence Rule 1 atomic honored)"
else
  fail "TC6 — scripts/tests/INDEX.md missing d1091 row (Cadence Rule 1 atomic violation per ADR-0055 §1)" "Add d1091 row to INDEX.md in this commit per sister-pattern d1082 TC6"
fi
