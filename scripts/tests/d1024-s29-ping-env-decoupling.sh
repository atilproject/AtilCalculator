#!/usr/bin/env bash
# d1024-s29-ping-env-decoupling.sh — STORY-S29 ping-env-decoupling d-test
#
...
#   TC4: AC1 — peer-poke.sh with TELEGRAM_BOT_TOKEN unset → exits 2 +
#        tmux-wake fires (inherits notify.sh option B behavior)
...
#
# Doctrinal home: Issue #1053 (canonical tracker) + Issue #1054 (d-test issue) +
#   Issue #1055 (impl issue). Sprint 29 W1 ping-env-decoupling cluster, owner
#   directive 2026-07-13 pickup-141.
#
# RED-first per ADR-0044: TC1, TC2, TC4 FAIL pre-impl (current notify.sh
#   `exit 1` on Telegram env-missing / API-fail, no tmux-wake fired — the
#   broken behavior the fix is closing). TC3 is a regression guard (happy
#   path was correct pre-impl and should remain so). TC5 is an OSC-2
#   regression guard (fallback path correctness).
# Post-impl (AC1 option B): all 5 TCs GREEN.
#
...
#   - d296 (peer-poke argv + usage discipline) — TC4 inherits notify.sh argv shape
#   - d320 (architect-authored stale_verdict contract shape) — exit-code
#     semantics + stderr structure
#   - d058 (work-stream aware — owner-mercy-gate contract) — fake-session
#     isolation pattern (no live peer pane touched)
#   - d1018 (S29-006 ADR-port parity) — RED-first d-test cadence, same
#     "all-RED-pre-impl, all-GREEN-post-impl" semantics
#   - d1020 (S29-010 workflow port-parity) — Sprint 29 d-test cadence,
#     same author lane (tester), Cadence Rule 1 atomic discipline
#
# Cross-refs:
...
SCRIPT_DIR_D1024="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT_D1024="$(cd "${SCRIPT_DIR_D1024}/../.." && pwd)"
NOTIFY_SH="${REPO_ROOT_D1024}/scripts/notify.sh"
PEER_POKE_SH="${REPO_ROOT_D1024}/scripts/peer-poke.sh"
AGENT_WAKE_SH="${REPO_ROOT_D1024}/scripts/agent-wake.sh"
...
TC1_WAKE_PROBE=$(capture_wake_probe "$SESSION_TC1" "test d1024 tc1 env-unset probe")
...
TC2_WAKE_PROBE=$(capture_wake_probe "$SESSION_TC2" "test d1024 tc2 invalid-token probe")
...
if [ -z "${TELEGRAM_BOT_TOKEN:-}" ] || [ -z "${TELEGRAM_CHAT_ID:-}" ]; then
    ...
    TC3_EXIT=$(TMUX_SESSION="$SESSION_TC3" \
        bash "$NOTIFY_SH" -l info -w -r architect "test d1024 tc3 happy-path probe" \
        2>"$TC3_STDERR" >/dev/null; echo $?)
    TC3_STDERR_CONTENT=$(cat "$TC3_STDERR")
    rm -f "$TC3_STDERR"

    TC3_WAKE_PROBE=$(capture_wake_probe "$SESSION_TC3" "test d1024 tc3 happy-path probe")
...
# -------------------------------------------------------------------------
# TC4: AC1 — peer-poke.sh with TELEGRAM_BOT_TOKEN unset → exits 2 + tmux-wake
# -------------------------------------------------------------------------
echo ""
echo "TC4: peer-poke.sh with Telegram unset → exit 2 + tmux-wake (inherits notify.sh option B)"
SESSION_TC4="d1024-tc4-$$"
...
TC4_WAKE_PROBE=$(capture_wake_probe "$SESSION_TC4" "test d1024 tc4 peer-poke env-unset probe")

# RED-first expectations (peer-poke.sh exec's notify.sh, so inherits behavior):
# - exit code 2 (peer-poke.sh forwards notify.sh's exit)
...
# addresses; deferred to follow-up d-test (out of scope for d1024).
...
TC5_EXIT=$(TMUX_SESSION="$SESSION_TC5" \
    bash "$AGENT_WAKE_SH" orchestrator "test d1024 tc5 osc2 fallback probe" \
...
echo ""
echo "==================================="
echo "d1024-s29-ping-env-decoupling: $pass pass, $fail fail"
echo "==================================="
[ "$fail" -eq 0 ] || exit 1