...
#
# Verifies S34-005 AC2 dispatch: 10 .github/workflows/*.yml files in atilcan65/AtilCalculator
# have self-hosted runner labels changed from `[self-hosted, Linux, X64, atilproject]` to
# `[self-hosted, Linux, X64, atilcan]`. Sister-PR of dev-studio-template PR #214.
#
...
# S34-005 AC2 dispatch scope (Issue #1225 audit-verified 10 files, mirror of dev-studio-template PR #214)
SCOPE_FILES=(
  "ai-pr-review.yml"
  "ci.yml"
...
# TC15: CHANGELOG.md entry present (sister-PR uses `Refs atilproject/AtilCalculator#1225` per ADR-0057 cross-repo format)
tc_num=$((tc_num + 1))
if grep -q "S34-005" "$CHANGELOG_FILE" && grep -qE "(Issue #1225|atilproject/AtilCalculator#1225)" "$CHANGELOG_FILE"; then
  run_tc "TC${tc_num}: CHANGELOG.md entry present (Sprint 34 S34-005 + Issue #1225 / atilproject/AtilCalculator#1225 cross-repo Refs)" 0
else
  run_tc "TC${tc_num}: CHANGELOG.md entry MISSING for Sprint 34 S34-005 Issue #1225 / atilproject/AtilCalculator#1225 cross-repo Refs" 1
fi

echo ""
total_tcs=$tc_num
if [ "$failed" -eq 0 ]; then
  echo "✅ All $total_tcs TCs GREEN — Sprint 34 S34-005 AC2 runner label atilcan verified (atilcan65/AtilCalculator sister-PR)"
  echo "   Issue #1225 (Closes #1225 anchor, parent dispatch Issue atilproject/AtilCalculator)"
  echo "   Sister-PR of dev-studio-template PR #214"
