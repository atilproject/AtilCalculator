#!/usr/bin/env bash
# d1025-s29-agent-wake-hotfix.sh — Phase A RED-first d-test on AtilCalculator
#
# Why this test exists
# --------------------
# Issue #1062 (Phase A) + Issue #1063 (Phase B) cluster — scripts/agent-wake.sh
# has 3 known defects that mask delivery failures and silently misroute to wrong
# panes. Phase A d-test ships BEFORE impl per ADR-0044 RED-first TDD.
#
# 3 fixes (Issue #1063 Phase B scope):
...
#   TC7 Cadence Rule 1 atomic — INDEX.md has d1025 row (ADR-0055 §1)
#
# Run: bash scripts/tests/d1025-s29-agent-wake-hotfix.sh
# Exit: 0 = all pass, 1 = at least one fail.
#
# Sister-pattern: d024-agent-wake.sh (existing source-coverage tests),
#                 d1024-s29-ping-env-decoupling.sh (Sprint 29 W1 cadence),
#                 d-retro-024 TC6 (Cadence Rule 1 INDEX.md attestation).
...
#!/usr/bin/env bash
# Mock tmux for d1025 RED-first testing
echo "$@" >> "${MOCK_LOG}"
case "$1" in
  has-session) exit 0 ;;
  list-panes)
...
if ! [ -f "$WAKE_SH" ]; then
  fail "agent-wake.sh missing" "expected $WAKE_SH (refs Issue #1062)"
...
# but TC3 only checks the mock log for send-keys entry. Sister-pattern: TC4 (line 170),
# TC5 (line 184) already use `|| true`. TC1 (line 126-129) wraps with `set +e`/`set -e`.
# Without this wrap, d-test's `set -e` (line 129 from TC1) crashes on WAKE_SH exit 1.
...
# TC7: Cadence Rule 1 atomic — INDEX.md has d1025 row (ADR-0055 §1)
# ============================================================================
section "TC7: Cadence Rule 1 atomic — INDEX.md has d1025 row (ADR-0055 §1)"
if grep -qE 'd1025.*-s29-agent-wake-hotfix|d1025.*agent-wake-hotfix' "$INDEX_MD"; then
  pass "INDEX.md has d1025 row (Cadence Rule 1 atomic)"
else
  fail "INDEX.md missing d1025 row" "expected 'd1025' row in $INDEX_MD per ADR-0055 §1 — file + INDEX.md row must land same commit"
fi
