#!/usr/bin/env bash
# d1138-agent-wake-fix-4b-lenient-verify.sh — Issue #1138 / ADR-0072 Fix 4b
#   regression guard (lenient capture-pane verify + hierarchical exit code).
#
# Why this test exists
# --------------------
# Issue #1138 documents 6/6 Sprint 31 cycles (cycles ~#2855, ~#2857, ~#2858 ×2,
# ~#2861) where `scripts/agent-wake.sh`'s Fix 3 verify produced FALSE failures:
#
#   - send-keys succeeded (message reached pane)
#   - capture-pane verify FAILED (timeout too tight / MSG_PREFIX drift)
#   - script exited 1 (false failure)
#   - actual wake DELIVERED (via GitHub artefact path, ADR-0033 dual-channel)
#
# Fix 4b (ADR-0072) addresses this with three additive changes:
...
# ADR-0044 RED-first TDD: pre-impl on main HEAD expected to FAIL on TC1-TC4.
...
#   - d862 (agent-watch orch lens fix, fake-gh-in-PATH pattern, Issue #862)
#   - d068b (WAKE_KEYS_GAP_SEC env override naming, Issue #935)
#   - d058 (claim WIP workstream, 9 TCs, fake-gh factory)
#   - d082 (S21-006 soul files template coverage, 5 TCs)
#
# Refs: Issue #1138 (P1 bug, scripts/agent-wake.sh Fix 4), ADR-0072 (Fix 4b
#       decision, leniency + hierarchical exit), ADR-0033 (dual-channel doctrine),
#       ADR-0044 (RED-first TDD), ADR-0049 (d-test framework ≥3 baseline + ≥5
#       with sister-pattern), ADR-0055 §1 (Cadence Rule 1 atomic — d-test +
#       INDEX.md row same commit), ADR-0057 (Closes anchor strict format).
...
# Pre-flight
command -v bash >/dev/null 2>&1 || { echo "ERROR: bash required for d1138" >&2; exit 127; }
command -v timeout >/dev/null 2>&1 || { echo "ERROR: GNU 'timeout' (coreutils) required for d1138" >&2; exit 127; }
[ -f "$WAKE_SH" ] || { echo "ERROR: agent-wake.sh not found at $WAKE_SH" >&2; exit 127; }

# Sentinel per ADR-0072 §D2 (literal 16 chars / 19 bytes UTF-8; "🔔" = 4 bytes)
SENTINEL='🔔 INBOX (dual-c'

# Setup test workspace
TEST_TMPDIR="$(mktemp -d /tmp/d1138-XXXXXX)"
...
case "$1" in
  ...
  list-panes)
    ...
    # filter matches our fake pane. Without this override, the script exits
    # at line 71-73 with "pane lookup failed" BEFORE reaching the verify
    # block — TC1/TC3-TC5 would fail for the wrong reason.
    PANE_IDX="${TMUX_PANE_INDEX_OVERRIDE:-0}"
    echo "%0 $PANE_IDX"
    exit 0
    ;;
  send-keys)
    # TC5: TMUX_SEND_FAIL env makes send-keys return non-zero (hard-fail path)
  ...
  capture-pane)
    ...
    if [ "${VERIFY_RESULT:-MATCH}" = "MATCH" ]; then
      printf '\033[1;33m%s test wake message d1138\033[0m\n' "$VERIFY_SENTINEL_OVERRIDE:-🔔 INBOX (dual-c"
    else
...
# Echoes the script's stdout/stderr capture locations for later assertions.
...
run_wake_capture() {
  ...
  (
    ...
    # Test invokes script with role=tester; production scripts/dev-studio-start.sh
    ...
    # production script exits at line 71-73 with "pane lookup failed" BEFORE
    # reaching the verify block.
...
# Pre-Fix-4b: script hardcodes `timeout 1` → log shows `timeout 1` → FAIL (RED).
...
(
  ...
  "$WAKE_SH" tester "🔔 INBOX (dual-c TC1 env-override scenario d1138 cycle ~#2864" >/dev/null 2>&1
)
if [ ! -s "$TMUX_LOG" ]; then
  ...
  fail "TC1 — WAKE_VERIFY_TIMEOUT_SEC env NOT honored (log shows '$TIMEOUT_SEEN')" \
    "expected: log shows 'timeout 99' (post-Fix-4b env override per ADR-0072 D1); current: '$TIMEOUT_SEEN' (RED — pre-Fix-4b hardcodes timeout 1)"
fi
...
# ADR-0072 §D2 mandates a hardcoded 16-char sentinel (UTF-8 byte length = 19
# due to "🔔" = 4 bytes). Static-grep on scripts/agent-wake.sh — the script
# MUST contain the literal sentinel as a constant, not derive it from MSG.
...
if grep -qF "$EXPECTED_LITERAL" "$WAKE_SH"; then
  ...
  fail "TC2 — VERIFY_SENTINEL literal '$SENTINEL' NOT in scripts/agent-wake.sh (uses dynamic MSG_PREFIX)" \
    "expected: literal '$EXPECTED_LITERAL' in scripts/agent-wake.sh (post-Fix-4b D2 sentinel); current: dynamic MSG_PREFIX derivation (RED — Fix 4b impl pending)"
fi
...
if grep -qF 'MSG_PREFIX="${MSG%%' "$WAKE_SH"; then
  fail "TC2.old-prefix-removed — old Fix 3 MSG_PREFIX derivation block still present (must be removed post-Fix-4b)" \
    "expected: no MSG_PREFIX dynamic derivation (D2 replaces with sentinel); current: Fix 3 derivation still in script (RED — Fix 4b impl pending)"
else
...
TC3_MSG="🔔 INBOX (dual-c TC3 verify-OK scenario d1138 cycle ~#2864 this message is intentionally long so its first 80-char prefix differs from the 16-char sentinel — pre-Fix-4b fails, post-Fix-4b matches"
...
TC4_MSG="🔔 INBOX (dual-c TC4 verify-FAIL scenario d1138 cycle ~#2864 verify should fail here because pane has no sentinel — pre-Fix-4b returns rc=1, post-Fix-4b returns rc=0+WARN"
...
if [ "$TC4_RC" = "0" ]; then
  ...
  fail "TC4 — verify-FAIL did NOT return rc=0 (rc=$TC4_RC, D3 hierarchical leniency broken)" \
    "expected: rc=0 (lenient per ADR-0072 D3 hierarchical exit); current: rc=$TC4_RC (RED — pre-Fix-4b treats verify-fail as hard fail rc=1)"
fi
...
# (lines 82-87 of scripts/agent-wake.sh) catches the rc and exits 1 + ERROR.
...
TC5_MSG="🔔 INBOX (dual-c TC5 send-keys-FAIL scenario d1138 cycle ~#2864"
...
# Summary
# ============================================================================
printf "\n${B}==== SUMMARY (d1138 — Issue #1138 / ADR-0072 Fix 4b lenient verify) ====${D}\n"
printf "  ${G}PASS${D}: %d\n" "$PASS"
printf "  ${R}FAIL${D}: %d\n" "$FAIL"
if [ "$FAIL" -gt 0 ]; then
  printf "  ${R}Failures${D}:\n"
  ...
  printf "\n${R}RED state confirmed${D} — Fix 4b impl in scripts/agent-wake.sh required (developer lane).\n"
  printf "  Action: developer opens impl PR with Closes #1138 (strict ADR-0057 anchor).\n"
  exit 1
fi
