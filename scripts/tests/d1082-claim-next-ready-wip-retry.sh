#!/usr/bin/env bash
# d1082-claim-next-ready-wip-retry.sh
#
# d1082 — Issue #1089 BUG regression guard: claim-next-ready.sh gh API error
#         on WIP query must retry-with-backoff, distinguish 4xx vs 5xx, and
#         surface stderr on final failure (currently aborts immediately + 2>/dev/null).
#
# Why this test exists
# --------------------
# scripts/claim-next-ready.sh lines 209 + 216 (both branches of WIP query)
# have:
...
#
# Live instance: cycle ~#1931 (2026-07-15T13:29:35+03:00), dev attempted to
# auto-claim Issue #1032 P0 via `scripts/claim-next-ready.sh` — script hit
...
# per ADR-0015 atomic 4-flag contract.
#
...
#   TC5 stderr surfaced on final failure — `claim.log`/stderr contains gh error (RED pre, GREEN post)
#   TC6 Cadence Rule 1 atomic — INDEX.md has d1082 row (ADR-0055 §1)
#
# Run: bash scripts/tests/d1082-claim-next-ready-wip-retry.sh
# Exit: 0 = all pass, 1 = at least one fail.
#
# Sister-pattern lineage:
#   - d1081 (Issue #1081 RETRO-024 silent-skip regression guard — same PR-cluster
#     cadence, same Issue #113 label-authority slot allocation, same ADR-0049
#     deviation-justification precedent for P1 cluster)
#   - d1043 (Issue #1086 --org/--repo flag-parser crash fix — same retry/guard
#     structural-correctness pattern, RETRO-005 #26 hygiene)
#   - d1020 TC2 (Issue #1067 four_tuple grep fix — recent scripts/ test gap close)
#   - d020a (Form C claim predicate race detection — sister at claim-next-ready.sh
#     interface)
#
# Refs: Issue #1089 (P1 sprint:current, agent:developer — cycle #2018 dev claim,
#       cycle ~#2041 orchestrator ACK + Sprint 30+ territorial guardrail on
#       AtilCalculator/scripts/* impl), ADR-0015 (atomic 4-flag hand-off —
#       manual workaround used during live instance cycle ~#1931), ADR-0038
#       §Layer 2 (claim-next-ready canonical home — current broken surface),
#       ADR-0044 (RED-first TDD doctrinal home — TC2-5 RED pre-impl),
#       ADR-0049 (≥5 TCs baseline — d1082 = 7 TCs met), ADR-0055 §1 (Cadence
#       Rule 1 atomic — d-test + INDEX.md row + impl same PR cluster — impl
#       sister-PR deferred pending owner signal per orchestrator Sprint 30+
#       territorial flag at cycle ~#2041), ADR-0059 (cluster-squash doctrine
#       — d1082 PR-cluster sister of d1081 PR #1092 cluster landed 15:30Z),
#       Issue #113 (label-authority — d1082 slot allocation: d1081 = RETRO-024
#       guard, d1082 = retry-guard sequential).
...
if ! command -v jq >/dev/null 2>&1; then
  echo "ERROR: jq required for d1082" >&2
  exit 127
fi
if [ ! -r "$CLAIM_SH" ]; then
...
case "$1" in
  repo) echo "atilproject/AtilCalculator" ;;
  api)
...
# where Issue #1089 lives, without touching real issues).
run_claim_wip_count() {
  : > "$MOCK_GH_CALL_LOG"
  MOCK_GH_CALL_LOG="$TEST_TMPDIR/gh-call.log" \
  MOCK_GH_FAIL_MODE="$1" \
...
# TC6 — Cadence Rule 1 atomic (ADR-0055 §1): INDEX.md has d1082 row.
# Sister-pattern d-retro-024 TC6 + d1025 TC7 (INDEX.md row attestation).
section "TC6 — Cadence Rule 1 atomic (INDEX.md d1082 row present)"
if grep -q "d1082" "$INDEX_MD" 2>/dev/null; then
  pass "TC6 — scripts/tests/INDEX.md has d1082 row (Cadence Rule 1 atomic honored)"
else
  fail "TC6 — scripts/tests/INDEX.md missing d1082 row (Cadence Rule 1 atomic violation per ADR-0055 §1)" "Add d1082 row to INDEX.md in this commit per sister-pattern d1025 TC7"
fi
