...
# --- Write ~/.dev-studio-env (STORY-S21-010 AC3 / Issue #642) -------------
#
# Why this exists
# ---------------
# STORY-S21-010 AC3 + architect 9-Lens (g) HIGH amendment: init must write a
# per-user shell-sourceable env file at ~/.dev-studio-env with the three values
# downstream scripts depend on (GITHUB_OWNER, GITHUB_REPO, HUMAN_OWNER_NAME).
# This lets scripts/source env-fish consume these values without re-running gh
# or re-parsing git origin on every invocation, and gives tests/d642 TC6+TC7+
# TC8 a stable surface to assert against.
#
# Security (arch 9-Lens (g) HIGH): file MUST be mode 0600. The file may be
# sourced by user-shell scripts and leaked into process listings; 0644 (the
# default umask) would expose the human owner's display name to other local
# users, which is the leak vector the 9-Lens flagged. We use umask 077 +
# explicit chmod 0600 + post-write stat attestation (arch 9-Lens (j) live-state
# attestation) so a regression in any layer is detected at init time.
#
# Idempotency: re-running init overwrites the file. The mktemp+chmod+mv dance
# avoids a window where the file is world-readable between truncate and chmod.

write_dev_studio_env() {
  local env_file="${HOME}/.dev-studio-env"

  if [ "$DRY_RUN" = "1" ]; then
    log "skipping ${env_file} write (dry-run)"
    return 0
  fi

  # Fail loud if HOME is unset (defensive — usually inherited from shell).
  if [ -z "${HOME:-}" ]; then
    fail "HOME is unset; cannot determine env-file path"
  fi

  log "writing ${env_file} (chmod 600 per arch 9-Lens (g))"

  # Tight umask for this subshell only; restore after so the rest of init
  # isn't accidentally affected.
  local saved_umask
  saved_umask="$(umask)"
  umask 077

  # Atomic write: tempfile in the same dir as the final file (so mv is
  # same-filesystem, hence atomic), chmod before content write, mv to final.
  local tmp_file
  tmp_file="$(mktemp "${env_file}.XXXXXX")" \
    || { umask "$saved_umask"; fail "mktemp failed for env-file tmp"; }
  chmod 0600 "$tmp_file" \
    || { rm -f "$tmp_file"; umask "$saved_umask"; fail "chmod 0600 failed for env-file tmp"; }

  cat > "$tmp_file" <<EOF
# Generated by dev-studio-init.sh — do not edit by hand
# Re-generate by running: bash scripts/dev-studio-init.sh
# Source this from your shell:  . "\$HOME/.dev-studio-env"
export GITHUB_OWNER="${GITHUB_OWNER}"
export GITHUB_REPO="${GITHUB_REPO}"
export HUMAN_OWNER_NAME="${HUMAN_OWNER_NAME}"
EOF

  # shellcheck disable=SC2154  # $? is mv's exit code below
  mv "$tmp_file" "$env_file" \
    || { rm -f "$tmp_file"; umask "$saved_umask"; fail "mv failed for env-file"; }

  umask "$saved_umask"

  # Live-state attestation per arch 9-Lens (j): verify actual file perms
  # post-write. stat -c %a is GNU coreutils (CI / Linux); fall back to
  # stat -f %Lp for macOS parity (best-effort; CI runs on Linux anyway).
  local actual_perms
  actual_perms="$(stat -c %a "$env_file" 2>/dev/null \
    || stat -f %Lp "$env_file" 2>/dev/null \
    || echo unknown)"
  if [ "$actual_perms" != "600" ]; then
    fail "~/.dev-studio-env perms are ${actual_perms}, expected 600 (arch 9-Lens (g))"
  fi

  ok "${env_file} generated (perms=600, 3 env vars)"
}
...
main() {
  preflight
  resolve_values
  # STORY-S21-010 AC3: emit ~/.dev-studio-env (chmod 600) before any downstream
  # step that might source it (e.g. ensure_project_token in a future call site).
  write_dev_studio_env
  ensure_project_token
  render_all
  verify
