...
for role in $ROLES; do
  ...
  # Issue #1091 fix: any active WIP = NOT idle (override activity-signal check).
  # Self-referential FP prevention (live instance cycle ~#2272, orchestrator-captured):
  # claim via `gh issue edit --add-label status:in-progress` does NOT count as a
  # 'comment' signal (signal 2 misses label flip); just-claimed issue has no
  # linked PR yet (signal 1 = -1 missing); no commits on PR (signal 3 = -1
  # missing) → all 3 signals missing → is_idle=true → flagged idle within
  # minutes of claim. Fix per Issue #1091 body points 1+2:
  #   1. Cross-check status:in-progress label before flagging idle
  #   2. If role has ≥1 status:in-progress + agent:<role>, NOT flag idle
  # Sister-pattern: d1088 (query_stale_verdict owner-gate exemption), d1081
  # (claim-next-ready RETRO-024 silent-skip predicate). Doctrinel class:
  # heuristic-gap that conflates "no recent activity" with "no active work"
  # when the activity-signal taxonomy misses a state-change primitive.
